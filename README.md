...
Le terme $\\left(r^{2} + 2 r \\cos{\\left(\\Omega T_{e} \\right)} + 1\\right)$ au dénominateur de ces expressions valant $\\sin{\\left(\\Omega T_{e} \\right)}^2$ lorsque la dérivée s'annule, il reste positif. Dès lors, le disque unité ($r \\lt 1$) dans le plan complexe des $z$ est transformé en le demi-plan gauche ($\\sigma \\lt 0$) dans le plan complexe des $p$. La stabilité est donc préservée lors du changement de variable.
